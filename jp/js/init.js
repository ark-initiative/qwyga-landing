...
if( user_country === "United Kingdom"){
	console.log("User is from UK");
	/*const annual_price = document.getElementById("-t-pricingpricing2price-");
	console.log(annual_price.innerHTML);
	annual_price.innerHTML = "£6.66";

	const annual_full_price = document.getElementById("-t-pricingpricing2belowaction-");
	annual_full_price.innerHTML = "£79.99 (every 12 months)";

	const quarterly_price = document.getElementById("-t-pricingpricing4price-");
	console.log(quarterly_price.innerHTML);
	quarterly_price.innerHTML = "£8.33";

	const quarterly_full_price = document.getElementById("-t-pricingpricing4belowaction-");
	quarterly_full_price.innerHTML = "£24.99 (every 3 months)";

	const monthly_price = document.getElementById("-t-pricingpricing1price-");
	console.log(monthly_price.innerHTML);
	monthly_price.innerHTML = "£9.99";*/
} else if ( user_country === "Australia" ){
	console.log("User is from Australia");
	const annual_price = document.getElementById("-t-pricingpricing2price-");
	console.log(annual_price.innerHTML);
	annual_price.innerHTML = "$10.83";

	// Babbel is: $83.40 per year
	// Rosetta Stone is: $126-$170
	// Pimsleur: $199.95 (all languages)
	// Memrise is: $89.99
	const annual_full_price = document.getElementById("-t-pricingpricing2belowaction-");
	annual_full_price.innerHTML = annual_full_price.innerHTML.replace("95,99€", "$129.99"); // = 78,37 EUR
	annual_full_price.innerHTML = annual_full_price.innerHTML.replace("€95.99", "$129.99");
	// --> MAKE THIS 129.99 (46% Rabatt)

	const quarterly_price = document.getElementById("-t-pricingpricing4price-");
	console.log(quarterly_price.innerHTML);
	quarterly_price.innerHTML = "$13.33";
	
	// Babbel is: $29.85 for 3 months
	// Rosetta Stone is: $47.97
	// Pimsleur: $49.95 (all languages)
	const quarterly_full_price = document.getElementById("-t-pricingpricing4belowaction-");
	quarterly_full_price.innerHTML = quarterly_full_price.innerHTML.replace("29,99€", "$39.99"); // = 24,1 EUR
	quarterly_full_price.innerHTML = quarterly_full_price.innerHTML.replace("€29.99", "$39.99");
	// = 34% Rabatt

	// Pimsleur is: $14.95 for audio of one language
	// Memrise is: $8.49
	const monthly_price = document.getElementById("-t-pricingpricing1price-");
	console.log(monthly_price.innerHTML);
	monthly_price.innerHTML = monthly_price.innerHTML.replace("12,99€", "$19.99"); // = 12,05 EUR
	monthly_price.innerHTML = monthly_price.innerHTML.replace("€12.99", "$19.99");

	// Get price ribbons
	const price_ribbons = document.getElementsByClassName("ribbon");
	console.log("Ribbons:", price_ribbons);
	for( let i = 0; i < price_ribbons.length; i++ ){
		const ribbon = price_ribbons[i];
		console.log(ribbon.innerHTML);
		if( ribbon.innerHTML.includes("39%") ){
			ribbon.innerHTML = ribbon.innerHTML.replace("39%", "46%");
		} else {
			ribbon.innerHTML = ribbon.innerHTML.replace("23%", "34%");
		}
	}
}